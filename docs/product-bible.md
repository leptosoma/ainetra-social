...
Creative Campaign applies a small sector policy selected automatically from the business sector: `HOSPITALITY_STANDARD`, `FOOD_STRICT_AUTHENTIC`, or `HEALTH_STRICT_COMPLIANCE`. Unknown sectors take the strictest safe default. The precedence is **authenticity policy > confirmed business facts > sector policy > brand style > creative freedom**. A design never becomes evidence of a real product, person, place, treatment result, or service. Its text must trace to current canonical confirmed facts when accepted, and a human must explicitly keep it. Health designs also require a separate acceptance and reject guarantee, risk-free, certain-success, success-rate, before/after, and patient-proof claims even when a fact was confirmed. Until semantic visual verification exists, these policies permit deterministic graphic templates but no generative imagery. This is a product safety boundary, not a jurisdiction-specific legal compliance guarantee.
