...
The **OSS Architecture Spike** is documented in [oss-architecture-spike.md](oss-architecture-spike.md). FullCalendar Standard is now used for the P5.5A desktop calendar; the Ainetra-owned mobile Today/3-day/Week and capture UI shipped in P5.5B. Keep Sharp, defer BullMQ/tusd/imgproxy, and use bounded FFmpeg tooling when video processing begins. Phase 6 owns a `PublishingAdapter` and transactional publication intent; tenant-safe reconciliation is still planned. Postiz remains a pilot behind the adapter subject to legal/security/operations gates; native Meta is the first production Instagram/Facebook path. Mixpost Lite does not meet Instagram/API needs, and paid-tier SaaS rights require separate review. Phase 6 is in progress.
...
- **P6-03 — Native Meta Submission (DONE; live Meta publishing not validated):** narrow Instagram single-JPEG and Facebook Page text/single-photo delivery through the adapter for an explicitly due-now intent; normalize definitive and unknown results, preserve remote IDs and idempotency evidence. No scheduled worker. Packet: `claude-tasks/P6-03-native-meta-submission.md`.
- **P6-04 — Scheduled PostgreSQL Worker (DONE; live Meta publishing not validated):** claim due intents with leases, bounded retry for definite safe failures, crash-safe processing and operational visibility using the P6-03 execution unit. No BullMQ. Packet: `claude-tasks/P6-04-postgres-scheduled-worker.md`.
