...
import { readBrandStyleOutput } from "@/features/brand-style/service";

// Karar verilmeden önce marka stili çıktısını önizlemek için. Saklanan çıktı ayrıca bir MediaAsset
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const output = await readBrandStyleOutput(user.id, id);
