...
async function expireDueCaptureRequests(businessId: string) {
  await prisma.captureRequest.updateMany({
    where: { businessId, status: "OPEN", dueAt: { lt: new Date() } },
    data: { status: "EXPIRED" },
  });
}

export async function listCaptureRequests(userId: string, businessId: string, options: { scope?: "TODAY" | "WEEK" | "ALL" } = {}) {
  await requireMembership(userId, businessId);
  await expireDueCaptureRequests(businessId);
  const scope = options.scope ?? "WEEK";
  const rangeEnd = scope === "TODAY" ? addDaysUtc(new Date(), 1) : scope === "WEEK" ? addDaysUtc(new Date(), 7) : null;
