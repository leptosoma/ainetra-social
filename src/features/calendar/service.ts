...
async function loadProjectionInputs(businessId: string, range: { from: string; to: string }, timeZone: string) {
  const [items, posts] = await Promise.all([
    ...
    prisma.scheduledPost.findMany({
      where: { businessId, scheduledAt: { gte: zonedDayStart(range.from, timeZone), lt: zonedDayStart(range.to, timeZone) } },
      select: {
  ...
  const planItems: ProjectionPlanItem[] = items.map((item) => ({
    id: item.id,
  ...
  const scheduledPosts: ProjectionScheduledPost[] = posts.map((post) => ({
    id: post.id,
